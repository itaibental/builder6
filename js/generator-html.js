    ...
    build: function(studentName, questions, instructions, examTitle, logoData, solutionDataUrl, duration, unlockCodeHash, parts, teacherEmail, driveLink, projectData, theme) {
        ...
        const sectionsHTML = parts.map((p, idx) => {
            ...
            return `<div id="part-${p.id}" class="exam-section ${idx===0?'active':''}">
                <h2 style="color:${theme && theme.header ? theme.header : '#2c3e50'}; border-bottom:0.3vh solid #3498db; font-weight: 700; padding-bottom:1vh; margin-bottom:3vh;">${p.name}</h2>
        ...
        const cloudSaveArea = `
            <div style="text-align:center; margin: 30px 0; background: #ebf5fb; padding: 20px; border-radius: 12px; border: 1px dashed #3498db;">
                ...
                <button type="button" onclick="saveProgressToCloud()" id="btnCloudSave" style="background: #3498db; color: white; padding: 12px 25px; border: none; border-radius: 30px; font-size: 1.1rem; cursor: pointer; transition: 0.2s;">
                    💾 שמור התקדמות בענן
                </button>
            </div>
        `;
        ...
        /* Question Content Styles - Ensure Selectable */
        .q-content, .sub-q-text, .instructions-text, .part-instructions {
        ...
        <div id="startScreen">
            ...
            <div class="sound-check-box">
                <p class="sound-check-text">
                    🔊 <strong>בדיקת שמע:</strong> ודאו שאתם שומעים את הצליל באוזניות.
                </p>
        ...
        <div class="container" id="mainContainer" style="filter:blur(5px);">
            ...
            <div style="text-align:center;margin-top:50px;border-top:1px solid #eee;padding-top:20px;">
                <div class="student-submit-area"><br><button onclick="submitExam()" style="background:#27ae60;color:white;padding:15px 30px;font-size:1.2em;border:none;border-radius:30px;">הגש בחינה באופן סופי</button></div>
            </div>
        </div>
        <script>
        ...
        // NEW: פונקציה לשמירת התקדמות ל-Firebase מתוך המבחן
        async function saveProgressToCloud() {
            const btn = document.getElementById('btnCloudSave');
            btn.innerText = "⏳ שומר נתונים לענן...";
            btn.disabled = true;
            ...
            // במערכת מקוונת אמיתית נשתמש במזהים שנשמרו ב-localStorage מהלוגין
            ...
            console.log("Saving to Cloud Payload:", payload);

            try {
                // הדמיה של שליחה מאחר וה-Firebase SDK לא מיובא ישירות ב-iframe התלמיד בקוד זה, 
                // בגרסת ה-run_exam.html הייעודית נייבא את ה-CloudService.
                // כרגע אנו שומרים ל-localStorage לצרכי הדגמה אם אין חיבור.
                localStorage.setItem('cloud_backup_' + examID, JSON.stringify(payload));
                
                setTimeout(() => {
                    btn.innerText = "✅ ההתקדמות נשמרה בהצלחה בענן";
                    btn.style.background = "#27ae60";
                    setTimeout(() => {
                        btn.innerText = "💾 שמור התקדמות בענן";
        ...
        function submitExam(){
            ...
            // עדכון סטטוס 'submitted' ב-Firebase
            saveProgressToCloud();
        }
